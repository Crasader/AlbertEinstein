...
void *JsonAllocator::allocate(size_t n, size_t align)
{
	...
	z->end = p + n;
	if (zone_size <= JSON_ZONE_SIZE || head == NULL)
	{
		z->next = head;
		head = z;
	}
		...
		*endptr = str++;
		switch (**endptr)
		{
			...
			case '[':
				...
				stack[top] = (JsonList){JSON_TAG_ARRAY, JsonValue(JSON_TAG_ARRAY, NULL), NULL};
				continue;
			case '{':
				if (++top == JSON_STACK_SIZE) return JSON_PARSE_STACK_OVERFLOW;
				stack[top] = (JsonList){JSON_TAG_OBJECT, JsonValue(JSON_TAG_OBJECT, NULL), NULL};
				continue;
			case ':':
				if (separator || stack[top].key == NULL) return JSON_PARSE_UNEXPECTED_CHARACTER;
				separator = true;
				continue;
			case ',':
				if (separator || stack[top].key != NULL) return JSON_PARSE_UNEXPECTED_CHARACTER;
