...
//  
    vector<std::string> vectDescription = LoadWayPointDescriptionToArray(this->wayPoint);
...
//    {            insertLabel(ccc3(72, 72, 72), ccp(30, 176*15 - 24 - i*20), false, vectDescription[i].c_str(), node, 12);//todo
  string tmpstring = "";
    for(int i =0; i < vectDescription.size();i++)
            {
                tmpstring = tmpstring + vectDescription[i];
            }
    
   // CCSize tamanho_texto = CCSizeMake(0, 0);
   // CCLabelTTF::create("", tamanho_texto, 0, "Thonburi", 20);
    if (altura>480) {
       insertText(ccc3(72, 72, 72), ccp(30, 60),CCSizeMake(270, 180), true, tmpstring.c_str(), node,"Thonburi", 16);
    }
    else
    {
        insertText(ccc3(72, 72, 72), ccp(30, 60),CCSizeMake(270, 100), true, tmpstring.c_str(), node,"Thonburi", 16);
    }
    
    //"O texto apresentado nesta área representa as principais caracteristicas do local visitado. Este texto não pode ser muito grande visto que não existe rolagem nesta região e em telas de 3.5 polegadas o texto seria cortado."
