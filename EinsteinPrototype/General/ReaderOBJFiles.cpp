//
//  ReaderOBJFiles.cpp
//  EinsteinPrototype
//
//  Created by Diego Mucciolo on 28/02/13.
//  Copyright (c) 2013 Farofa Studios. All rights reserved.
//

#include "ReaderOBJFiles.h"


void ObjReader::Render()
{
   // glClearColorx(0, 0, 255, 0);
    glDrawArrays(GL_TRIANGLE_STRIP, 0, lstVertices.size( ));
    //glDrawArrays(GL_TRIANGLE_STRIP, 0, lstVertices.size( ));
}

void ObjReader::SetupBuffers()
{
    glVertexAttribPointer(kCCVertexAttrib_Position, 3, GL_FLOAT, GL_TRUE, 0, &lstVertices);
    glVertexAttribPointer(kCCVertexAttrib_TexCoords, 2, GL_FLOAT, GL_TRUE, 0, &lstUV);
}


bool ObjReader::loadOBJ()
{
    std::vector<unsigned int> vertexIndices, uvIndices, normalIndices;
    std::vector<Vertex> temp_vertices;
    std::vector<UV> temp_uvs;
    std::vector<Vertex> temp_normals;
 
    FILE * file = fopen(CCFileUtils::sharedFileUtils()->fullPathFromRelativePath( "cube.obj" ), "r");
        if( file == NULL ){
            printf("Error in reading file");
            return false;
        }
        
        while( 1 ){
            
            char lineHeader[128];
            // read the first word of the line
            int res = fscanf(file, "%s", lineHeader);
            if (res == EOF)
                break; // EOF = End Of File. Quit the loop.
            
            // else : parse lineHeader
            
            if ( strcmp( lineHeader, "v" ) == 0 ){
               Vertex vertex;
                fscanf(file, "%f %f %f\n", &vertex.x, &vertex.y, &vertex.z );
                temp_vertices.push_back(vertex);
            }else if ( strcmp( lineHeader, "vt" ) == 0 ){
               UV uv;
                fscanf(file, "%f %f\n", &uv.u, &uv.v );
                uv.v = -uv.v; // Invert V coordinate since we will only use DDS texture, which are inverted. Remove if you want to use TGA or BMP loaders.
                temp_uvs.push_back(uv);
            }else if ( strcmp( lineHeader, "vn" ) == 0 ){
                Vertex normal;
                fscanf(file, "%f %f %f\n", &normal.x, &normal.y, &normal.z );
                temp_normals.push_back(normal);
            }else if ( strcmp( lineHeader, "f" ) == 0 ){
                std::string vertex1, vertex2, vertex3;
                unsigned int vertexIndex[3], uvIndex[3], normalIndex[3];
                int matches = fscanf(file, "%d/%d/%d %d/%d/%d %d/%d/%d\n", &vertexIndex[0], &uvIndex[0], &normalIndex[0], &vertexIndex[1], &uvIndex[1], &normalIndex[1], &vertexIndex[2], &uvIndex[2], &normalIndex[2] );
                if (matches != 9){
                   // printf("File can't be read by our simple parser :-( Try exporting with other options\n");
                   // return false;
                    
                }
                vertexIndices.push_back(vertexIndex[0]);
                vertexIndices.push_back(vertexIndex[1]);
                vertexIndices.push_back(vertexIndex[2]);
                uvIndices    .push_back(uvIndex[0]);
                uvIndices    .push_back(uvIndex[1]);
                uvIndices    .push_back(uvIndex[2]);
                normalIndices.push_back(normalIndex[0]);
                normalIndices.push_back(normalIndex[1]);
                normalIndices.push_back(normalIndex[2]);
            }else{
                // Probably a comment, eat up the rest of the line
                char stupidBuffer[1000];
                fgets(stupidBuffer, 1000, file);
            }
            
        }
        
        // For each vertex of each triangle
        for( unsigned int i=0; i<vertexIndices.size(); i++ ){
            
            // Get the indices of its attributes
            unsigned int vertexIndex = vertexIndices[i];
            unsigned int uvIndex = uvIndices[i];
            unsigned int normalIndex = normalIndices[i];
            
            // Get the attributes thanks to the index
            Vertex vertex = temp_vertices[ vertexIndex-1 ];
            UV uv = temp_uvs[ uvIndex-1 ];
            Vertex normal = temp_normals[ normalIndex-1 ];
            
            // Put the attributes in buffers
            lstVertices.push_back(vertex);
            lstUV.push_back(uv);
            lstNormal.push_back(normal);
            
        }
        
      SetupBuffers();
      return true;
}
